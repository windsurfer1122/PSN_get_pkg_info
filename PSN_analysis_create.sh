   ...
   do
     ...
     then
      ...
      for URL in $(cat "${URLSFILE}")
       do
        ...
        #
        { PSN_get_pkg_info.py --itementries --unknown -f 99 -- "${URL}" 3>&1 1>"${DIR}/_pkginfo/$(basename "${URL}").info" 2>&3 | tee -a "${ERRORLOG}" ; } || :
      done
    ...
    if [ "${DOFILES:-0}" -eq 1 ]
     then
      ...
      #export DIR
      #export ERRORLOG
      #{ printf '' | xargs -- find "${DIR}" -type f -name '*.pkg' ${EXTRA1:-} -exec sh -c 'PSN_get_pkg_info.py --itementries --unknown -f 99 -- "${1}" 3>&1 1>"${DIR}/_pkginfo/$(basename "${1}").list" 2>&3 | tee -a "${ERRORLOG}"' -- '{}' \; ; } || :
      { printf '' | xargs -- find "${DIR}" -type f -name '*.pkg' ${EXTRA1:-} -exec sh -c "PSN_get_pkg_info.py --itementries --unknown -f 99 ${EXTRA2A:-} ${EXTRA2B:-} ${EXTRA2C:-} -- \"\${1}\" 3>&1 1>\"${DIR}/_pkginfo/\$(basename \"\${1}\").info\" 2>&3 | tee -a \"${ERRORLOG}\"" -- '{}' \; ; } || :
